...
AccuKnox’s **KSPM Dashboard** can show on the order of **~25 configurable widgets**. When you **create a custom dashboard** in their UI, the app sends an HTTP request whose JSON body includes a `widgets` array of UUIDs (for example `{"name":"test","description":"…","widgets":["a9bbed6b-…",…],"is_default":false}`). Those values are **AccuKnox’s widget identifiers** (catalog / instance IDs in their backend). OpenCNAPP does **not** reuse those UUIDs at runtime; we use **stable slugs** in `dashboard/src/config/kspmDashboardWidgets.ts` and map tiles to **our** APIs.

### Git: “branch is up to date with `main`”

If **`feat/kspm-cluster-detail-inventory`** and **`main`** both point at the **same commit** (after merging the feature into `main` and optionally fast-forwarding the feature branch), GitHub/Git will correctly report the feature branch as **up to date with `main`**. That means there is **nothing left to merge** from that branch into `main`—the work is already on `main`. That is expected, not an error.

To continue work, create a **new branch** from `main` (e.g. `feat/kspm-dashboard-wire-widgets`) for the next PR.
...
- AccuKnox widget UUIDs below are **documentation-only** (reference from their platform’s request body); OpenCNAPP does not call AccuKnox APIs.

## AccuKnox `widgets` array (reference — from HTTP request body)

The following **25 UUIDs** were observed in the **JSON body** of an AccuKnox request when saving a **custom KSPM dashboard** (same order as in the request; **order may match widget selection order in the UI, not necessarily the visual layout**). AccuKnox does not publish a public map from UUID → widget title in our repo; to build a mapping you would need to correlate DevTools **Network** responses (e.g. widget catalog endpoints) with these IDs.

**Semantic mapping (UUID → title) is not stored here**—only the list for traceability.
